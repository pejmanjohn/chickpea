...
- Slack is the only human sign-in. Control-plane access is invitation-only after the installer becomes the first Chickpea Owner; Slack workspace roles do not grant Chickpea permissions.
- **Updates are manual**: the Deploy button clones this repo (it does not fork), so upgrading is a re-deploy.
...
The public Deploy button installs the slim runtime and Slack-native authentication in your own Cloudflare account:
...
See [Authentication and roles](docs/authentication.md) for Slack sign-in, exact-member invitations, sessions, and Chickpea roles. The hidden [Slack recovery runbook](docs/runbooks/slack-auth-recovery.md) explains the 15-minute, same-app credential and deployment-URL repair path; it cannot create or sign in a person.
...
- A single self-contained admin page. Better Auth verifies the Slack OIDC browser session; Chickpea re-resolves the exact Slack workspace/user binding and enforces `owner`, `admin`, or `member` permissions on every request. Suspending or removing someone revokes their sessions and personal tokens.
- Team administration uses one exact-Slack-member, seven-day invitation per pending teammate. An Owner shares the private capability through a trusted channel; redemption requires a fresh Slack OIDC proof for the invited `team_id` and `user_id`. Reissue, revoke, expiry, mismatch, and replay fail closed.
- Reusable Agents: name, model, primary instructions, enabled skills, remote MCP and HTTP API connections with explicit approvals, exact repository grants, Slack identity, Agent memory, and an enable toggle. Disabling an Agent blocks its DMs and the next event in every placed Channel thread.
...
- Agent and Channel edits apply on the next newly admitted Slack event without a restart, including in an existing thread. A TurnJob freezes the resolved configuration for that one response and all of its retries, so an in-flight answer cannot drift. DMs use the same next-turn live resolution. `CHICKPEA_LIVE_CHANNEL_CONFIG=false` is the emergency rollback for new Channel admissions only; it restores write-once thread snapshots without changing already admitted TurnJobs.
...
### Workspace management from Slack and coding agents

Chickpea exposes the same requester-bound management service through Flue tools in Slack and an OAuth-protected MCP at `/mcp`. A single request can create an Agent, add its instructions and skills, declare connector and repository requirements, create a Channel configuration, and place the Agent there. Safe creation and reversible edits apply immediately; deletion, removal, authority changes, credential replacement/removal, capability expansion, active-Agent disablement, and recipe overwrites return a requester- and client-bound confirmation.

Secrets never enter tool arguments. When Gmail, an MCP server, GitHub, a model provider, or a dedicated Slack identity needs credentials, the tool returns a one-use Chickpea setup link that expires after 24 hours. The holder does not sign into Chickpea: the fragment capability opens only the exact frozen action, is scrubbed before navigation, and cannot change its target or scopes. Completion posts a non-secret receipt to the original Slack thread, or DMs the initiating Slack user for coding-agent requests.

The management MCP can inspect and edit Agents, Channels, placements, instructions, skills, connections, repositories, dedicated Slack identities and DM routing, Agent/Channel memory, routines, provider status, and Owner-only team authority. It can create a named Slack identity and issue its private app-installation form in the same progressive batch; setup cancellation and retirement require confirmation. Portable recipes export the operational shape of selected Agents and Channel intent while excluding credentials, workspace and account identifiers, member authority, Slack identity, repository installation credentials, and memory. Import preview resolves conflicts as clone, update, or skip and compiles the chosen result into the ordinary change contract; update remains confirmation-gated.

See the [workspace management MCP runbook](docs/runbooks/workspace-management-mcp.md) for client setup, permissions, setup links, recipes, diagnostics, and rollout controls.
...
- A fresh Cloudflare deployment exposes only the private Slack setup capability until the installed workspace and first Owner are active. Afterwards, a valid Better Auth session is still insufficient without the exact active Chickpea Slack binding, membership, and role.
- Bounded channel context may be fetched to classify an eligible ambient message or build an admitted turn, but ignored ambient bodies are not copied into Work, Runs, transcripts, memory, or decision telemetry. There is no passive workspace-message index. Separately, explicit channel memory persists small team-authored notes, their revisions/audit envelope, scope lifecycle, and bounded selection epochs. Each promoted thread keeps its own agent transcript and dedupe claims; each admitted response records the effective configuration revision it froze.
- Every newly admitted Channel or DM event resolves the current Agent, placement, instructions, model, skills, connection approvals, repository grants, Slack identity, and applicable memory. The resulting TurnJob is immutable for that in-flight response and its retries; the next event resolves again. Reassignment rotates the Flue instance revision and rehydrates bounded Slack thread history instead of posting a false fresh-context notice.
...
Run `dist/server.mjs` on any host. State is file-backed SQLite. Set a stable `CHICKPEA_AUTH_SECRET`, terminate HTTPS in front of every non-loopback deployment, then run `npm run setup:link -- https://<host>` to mint the same private setup-link contract as Cloudflare. On first credential write, Node atomically creates a mode-0600 credential keyring at `<SLACK_STATE_DB_PATH>.credential-keyring.json` (or `CHICKPEA_CREDENTIAL_KEYRING_PATH`). Back up and restore that file together with the state database; losing it closes the product into deployment recovery, and rotating `CHICKPEA_AUTH_SECRET` does not replace it. Point Slack at the request URL generated by `/admin`. Both targets run the same source — `src/config/state-backend.ts` picks SQLite or Durable Object state at runtime.
...
| `CHICKPEA_LIVE_CHANNEL_CONFIG` | internal rollback only | On by default: each newly admitted Channel event resolves current workspace configuration and freezes it only for that response and its retries. `false` restores write-once thread snapshots for new Channel admissions; already admitted TurnJobs do not change. |
...
| `CHICKPEA_CREDENTIAL_KEYRING_PATH` | optional on file-backed Node; required with in-memory state | Path to the independently auto-minted, mode-0600 Slack credential keyring. Keep it outside SQLite, and back it up and restore it with the state database. Cloudflare uses preserved versioned Worker secret slots instead. |
| `CHICKPEA_RECOVERY_TOKEN` | optional break-glass recovery | Temporary 32-byte deployment capability for one 15-minute, browser-bound repair of the unchanged Slack app/workspace credentials and URLs. It cannot create identity authority or issue a session; replace or delete it after use. |
...
- **Authentication and authorization are separate.** Slack OIDC proves the person, Better Auth owns the provider account and browser session, and Chickpea owns the exact workspace binding, invitations, memberships, roles, recovery, and audit.
- **The current schema is fresh-only.** There is no supported password, Cloudflare Access, shared-token, or private pre-release migration target. Use an empty `AUTH_DB` whose reviewed `0001` digest matches this artifact.
