...
| **Files and charts** | Agents attach final or revised sandbox deliverables, including Markdown, through their Slack reply. Scratch files stay private, and explicit no-attachment requests are respected. Missed exports get one repair attempt; files that cannot be attached are identified in the reply. Direct Slack installs support files up to 8 MiB; keep shared-app files below 700 KiB to fit the gateway request limit. Works without a coding sandbox or repository grant on Cloudflare and Node. Agents can also render bar, line, or pie charts as PNG images with Latin bitmap labels, up to 40 categories and 6 series, and numeric magnitudes from 1e-9 to 1e15 or zero. | |
