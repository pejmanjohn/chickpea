...
- **Slack is an adapter, not the runtime aggregate.** Canonical Work, Binding, Run, RunExecution, safe content, and audit records are channel-neutral. Slack owns Slack coordinates, rendering, and delivery. The authenticated Sessions inspector reads the canonical ledger, and a future web client can submit and render through the same lifecycle without impersonating a Slack thread. This release does not yet ship operator-originated web sessions.
