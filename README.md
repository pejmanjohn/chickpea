...
Secrets never enter tool arguments. When Gmail, an MCP server, GitHub, or a model provider needs credentials, the tool returns a one-use Chickpea setup link that expires after 24 hours. The holder does not sign into Chickpea: the fragment capability opens only the exact frozen action, is scrubbed before navigation, and cannot change its target or scopes. Completion posts a non-secret receipt to the original Slack thread, or DMs the initiating Slack user for coding-agent requests.

The management MCP can inspect and edit Agents, Channels, placements, instructions, skills, connections, repositories, Slack identity references, Agent/Channel memory, routines, provider status, and Owner-only team authority. Portable recipes export the operational shape of selected Agents and Channel intent while excluding credentials, workspace and account identifiers, member authority, Slack identity, repository installation credentials, and memory. Import preview resolves conflicts as clone, update, or skip and compiles the chosen result into the ordinary change contract; update remains confirmation-gated.
