...
- Team administration uses exact-email, seven-day, show-once invitation links. A public no-store handoff moves the secret from the URL fragment into same-tab storage and removes it from browser history. New users choose a password; existing users sign in normally and resume. Rotate, revoke, expiry, mismatch, and replay fail closed. Chickpea sends no invitation email.
...
- A read-only channel summary showing the attached profile, install-wide Slack identity, and layered instructions. Advanced shows the resolved model, provider, and short config snapshot hash; the profile's Skills, Connections, and Repositories tabs show its capability grants.
