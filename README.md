...
The hard deployment defaults are 100 active routines, 20 per channel, 240 scheduled starts/day, 10 run-now starts/day, four starts per rolling 15 minutes, four concurrent runs deployment-wide, one active run per routine, a one-hour minimum interval, a 15-minute admission grace/deadline, and 25 due claims per heartbeat. Schedule validation calculates the maximum rolling-day rate across 370 days, while collision previews persist only the next three fires or 48 hours of hourly fires and refresh as each slot advances. The four-start rolling limit is enforced again transactionally when work is actually queued, so an older routine can never bypass it after the preview horizon. Scheduled Work exposes these bounds. Product-owned run and audit metadata is retained for 365 days; confirmed deletion immediately scrubs the saved task from the routine and its product revisions. Slack messages, provider processing/logs, backups, and Flue's separate run history have their own retention and cannot be retracted by Chickpea.
