...
export function routineExecutionInstructions(
  ...
  return [
    ...
    'When the saved task asks for a file, chart, image, or export, follow the saved task and Agent instructions using the available tools. Use `post_artifact` to attach files you create in the sandbox. An attached: true result prepares the file for this reply; Chickpea publishes it with your returned message under your Agent identity at the saved destination. Return the text result in message. Do not invent file links or claim a file is ready when staging failed.',
    direct
