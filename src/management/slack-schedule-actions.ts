...
// The tool may omit unchanged edit fields. Only a complete validated operation enters the ledger.
type ScheduleSave = Extract<ManagementOperation, { kind: 'save_routine' }>;
type ScheduleFields = 'name' | 'description' | 'taskText' | 'schedule' | 'timezone' | 'outputPolicy';
export type SlackScheduleToolOperation =
  | (Omit<ScheduleSave, ScheduleFields> & { [K in ScheduleFields]?: ScheduleSave[K] | undefined })
  | Exclude<SlackScheduleManagementOperation, { kind: 'save_routine' }>;
...
export async function invokeSlackScheduleAction(input: {
  ...
  const bound = await bindScheduleOperationToRequester(input.signal, operation, input.dependencies.routines);
  const [validated] = validateManagementOperations([bound]) as [SlackScheduleManagementOperation];
...
async function bindScheduleOperationToRequester(
  ...
  if (operation.kind !== 'save_routine' || operation.routineId) {
    ...
    previous = await routines.getRoutine(operation.routineId);
    if (!previous || previous.deletedAt !== null || previous.workspaceId !== signal.workspaceId ||
        previous.channelId !== signal.channelId ||
        (signal.conversationKind === 'im') !== (previous.destination.kind === 'direct_thread')) {
      throw new ManagementError('invalid_request', 'The scheduled work was not found.');
    }
  }
  ...
  if (previous && previous.version !== operation.expectedVersion &&
    ...
    // Resolve omitted fields from the version the caller edited, not whatever
    // happens to be current on a replay after this or another action applied.
    const revision = (await routines.listRevisions(previous.id)).find(
      (entry) => entry.version === operation.expectedVersion,
    );
    if (!revision?.definition) throw new ManagementError('invalid_request', 'The scheduled work changed. Inspect it again before editing.');
  ...
  if (signal.requesterTimezone) {
    try {
      new Intl.DateTimeFormat('en-US', { timeZone: signal.requesterTimezone });
      profileTimezone = signal.requesterTimezone;
    } catch { /* An invalid Slack profile zone is unavailable, not a guessed default. */ }
  }
  // Relative delays are elapsed time and do not need a local wall-clock zone.
  const timezone = operation.timezone ?? previous?.timezone ?? profileTimezone ?? (schedule?.kind === 'in' ? 'UTC' : undefined);
  if (!timezone) {
    throw new ManagementError('invalid_request', "A timezone is required and none is available from the requester's Slack profile.");
  }
  const name = operation.name ?? previous?.name;
  const description = operation.description ?? previous?.description;
  const taskText = operation.taskText ?? previous?.taskText;
  if (name === undefined || description === undefined || taskText === undefined || schedule === undefined) {
    throw new ManagementError('invalid_request', 'Name, description, task text, and schedule are required.');
  }
  return { ...operation, name, description, taskText, schedule, timezone,
    outputPolicy: operation.outputPolicy ?? previous?.outputPolicy ?? 'post' };
