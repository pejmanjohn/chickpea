...
export const CONNECT_CLIENTS: readonly ConnectClient[] = Object.freeze([
  {
    ...
    where: 'terminal',
    language: 'bash',
    snippet: (url) => `claude mcp add --transport http --scope user ${MCP_SERVER_NAME} ${url}`,
    login: `Run \`/mcp\` inside Claude Code, choose \`${MCP_SERVER_NAME}\`, and follow the browser sign-in.`,
    restart: 'Servers added from the terminal appear in the next Claude Code session. User scope makes it available outside the installation checkout. Use project scope only when explicitly requested.',
  },
  {
    id: 'codex',
    title: 'Codex',
...
export function connectMarkdown(origin: string): string {
  ...
  return [
    ...
    '',
    `Read the resource \`${AGENT_AUTHORING_GUIDE_URI}\`. If your client shows Chickpea\'s prompts as slash commands, suggest \`/mcp__${MCP_SERVER_NAME}__new-agent\`, which includes curated starter suggestions. Otherwise ask: "What should your first teammate do for your team?" The guide does not contain the starter catalog; do not invent catalog entries. Nothing is created until the person chooses a job.`,
    '',
    'No separate local skills are required. In Codex, ask for the desired Chickpea change in plain language; MCP prompts do not appear in its `$` skills picker.',
    '',
    '## Good to know',
    '',
