...
export const AGENT_AUTHORING_GUIDE_VERSION = '1.0.34' as const;
...
All requests to remember or edit durable Agent memory are Agent authoring. Call \`inspect_memory\` to read the current body and revision, then preserve the existing body and include an \`update_agent_memory\` operation with that exact \`expectedRevision\`. Preserve every actionable destination or reference link and condition in the current request: copy exact URLs unchanged rather than substituting opaque IDs, names, or labels, and retain scope, exceptions, override conditions, and negative constraints, including when another tool or destination should take precedence. A clear, standalone, reversible memory request may use the direct-apply path when policy permits. If the same turn also asks for standing behavior, a skill, access, identity, model, reach, editing authority, or scheduled work, include the memory operation in the same read-only proposal as the other primitives; never partially apply the turn.
