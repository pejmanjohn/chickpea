...
export const AGENT_AUTHORING_GUIDE_VERSION = '1.0.38' as const;
...
export const AGENT_AUTHORING_ROUTER_INSTRUCTION = [
  'Always activate the `agent-authoring` skill for requests to create an Agent, edit an Agent, remember or edit Agent memory, explore Agent roles or workflows, ask a capability question about Agent configuration or design, or create or revise a skill. Do not activate it merely to answer which native or connected-service actions are mounted for the current turn; answer those questions from the mounted runtime declarations and tool descriptions.',
  ...
  'Preview-first or wait-for-approval Agent creation is read-only exploration, even when fully specified. Show a textual draft; call neither apply_workspace_changes nor propose_workspace_changes. Only a later authenticated approval reaches commit posture.',
  ...
  'Use import_skill for an explicit public GitHub skill source in the authenticated current message; it pins and installs one bounded scriptless skill immediately or returns an exact selection or replacement clarification. Use manage_agent_skill for one explicit enable, disable, or removal of a named existing skill; it applies only that reversible local change without a proposal.',
  'For Agent brainstorming or Agent-configuration and design capability questions involving services or connections, call `inspect_workspace` in the same turn before naming services or availability; do not answer from general knowledge or defer inspection.',
...
Use this guide when a requester wants to explore, create, onboard, or edit a Chickpea Agent, asks what its configuration or design could support, or wants to create or revise a reusable Agent skill. Do not use it merely to answer which native or connected-service actions are mounted for the current turn; answer those questions from the mounted runtime declarations and tool descriptions. Use it for scheduled work when that work is part of a compound Agent-configuration request or when the requester asks to delete scheduled work. The management service is the only mutation authority. This guide helps you reason and compose; tools validate, authorize, preview, confirm, and apply.
...
Every new save_routine or changed task must declare requiredConnectionAccountIds, using the exact ready enabled account IDs from inspect_workspace for the owning Agent. Declare [] for work without external connections; never default to all accounts. Metadata-only edits omit this field and preserve the saved set, which inspect_routines returns. Ask when the required account is ambiguous. A clear instruction to continue work at a future time is an explicit schedule request and does not need to say schedule. For example, “Check this again in 5 minutes and tell me anything new” means fresh one-time scheduled work, not an edit to an existing routine. Relative timing stays relative so the service computes the exact future instant from its trusted clock. In a compound request, represent the schedule with the same \`save_routine\` operation used by the shared command, but keep it inside the proposal required by the other changes. Never use the compound path to add an approval round trip to a standalone clear schedule action.
...
Use \`propose_workspace_changes\` only for a typed Chickpea workspace configuration operation accepted by its schema. It does not execute actions in Slack Lists or connected services; use an available native or connector tool for those actions. Approval cannot make an unsupported operation available. Within that schema, use it for generated or inferred skill content, compound or multi-field follow-on configuration changes, capability expansion, reach or authority changes, destructive configuration changes, or otherwise consequential configuration edits. Never use it for Agent creation; the service rejects creation proposals. A proposal is read-only: it normalizes the exact typed operations, shows a bounded visible diff and missing setup, records the guide version, and returns a requester-, conversation-, and acting-Agent-bound handle. The proposal preview is the review; proposing never requires its own approval. Standalone scheduled work does not belong to this path except for deleting a routine through the typed \`delete_routine\` configuration operation.
...
For a typed workspace configuration operation, an explicit requester command executes without another confirmation when the trusted service can derive its exact effect from the authenticated current message and proves it is authorized, reversible, local-only, and free of authority, reach, credential, or capability effects. Standalone base Agent creation is an explicit product exception and also applies immediately. Confirmation remains required for generated skill content, inferred or compound follow-on configuration changes, authority or reach changes, destructive configuration changes, and other configuration effects the service classifies as consequential. Native and connected-service tools follow their own availability and confirmation policies. Ambiguity calls for clarification, not approval. For one explicit enable, disable, or removal of a named existing skill, call \`manage_agent_skill\`. The service reads current state, changes only that skill, preserves every other skill, applies immediately, and returns a receipt plus undo. Never construct a replacement \`skills\` array or route that exact command through \`propose_workspace_changes\`. Existing setup, permission, revision, reach, and destructive-configuration gates always win.
...
const agentAuthoringSkill = defineSkill({
  ...
  description: 'Explore, create, onboard, or edit a Chickpea Agent, remember or edit Agent memory, answer questions about Agent configuration or design capabilities, or create or revise reusable Agent skills. Do not activate for questions limited to native or connected-service actions mounted for the current turn. Also use for scheduled work when it is part of a compound Agent-configuration request.',
