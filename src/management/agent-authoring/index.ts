...
export const AGENT_AUTHORING_GUIDE_VERSION = '1.0.26' as const;
...
export const AGENT_AUTHORING_ROUTER_INSTRUCTION = [
  ...
  'For a request to install a public GitHub-hosted skill, use import_skill with the source present in the authenticated current message instead of browsing or copying the skill manually. The trusted service pins the inspected commit, applies one bounded scriptless skill immediately, and returns the receipt; follow its exact selection or replacement clarification, not a second approval.',
...
Use \`propose_workspace_changes\` for new-Agent creation and for generated or inferred skill content, compound or multi-field changes, capability expansion, reach or authority changes, destructive actions, external consequential writes, or otherwise consequential edits. A proposal is read-only: it normalizes the exact typed operations, shows a bounded visible diff and missing setup, records the guide version, and returns a requester-, conversation-, and acting-Agent-bound handle. The proposal preview is the review; proposing never requires its own approval. Standalone scheduled work does not belong to this path except for irreversible deletion, which always does.
...
The request itself is approval only when the trusted service can derive the exact command from the authenticated current message and proves it is an authorized, reversible, local-only change with no authority, reach, credential, capability, or third-party effects. \`apply_workspace_changes\` may immediately enable, disable, or remove one named existing skill when the current message explicitly says so. New or generated skill content, inferred changes, ambiguous targets or replacements, compound changes, authority or reach changes, external writes, and irreversible actions use a proposal or a purpose-built clarification flow. Ask a clarifying question instead of asking for approval when the missing decision is which object or replacement the requester intends. Existing setup, permission, revision, reach, destructive, and external-action gates always win.
