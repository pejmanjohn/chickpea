...
const PREAMBLE = [
  '> **You are reading this over the Chickpea MCP.** You are a coding agent acting as the signed-in person, not an Agent inside Slack. Where this guide says "the requester", it means the person in your conversation. Show them `presentation.markdown` from each result, never `presentation.slack`. Mutation receipts carry `links.admin` (the Agent in Admin) and `links.slack` (Chickpea in Slack); share them so the person can open the Agent or try it by mentioning its @handle. A model provider key is added through `prepare_provider_setup` (Owner or Admin): give the person its handoff link and never handle the key. Results and errors that name something only Admin can change carry `links.admin` to the exact Settings section. The Slack-only tools `update_agent_memory`, `manage_scheduled_work`, and `request_chickpea_handoff`, and the `connectorMentions` creation hint, do not exist on this connection; the coding-agent equivalent is given inline below.',
  '',
  '',
].join('\n');
