...
export function workspaceManagementInstructions(baseUrl?: string): string {
  ...
  return [
    ...
    '4. Never ask for, accept, or pass along secrets: API keys, tokens, OAuth codes, passwords. To connect a service, call prepare_connector_setup; for a model provider key (Owner or Admin), prepare_provider_setup. Give the person the handoff link.',
    `5. Admin-only today: Agent avatars (Agent page, then Configure) and these Settings sections under ${adminUrl}: ${settings}. Send the person the link; results that touch them carry links.admin.`,
