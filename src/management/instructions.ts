...
export function workspaceManagementInstructions(baseUrl?: string): string {
  ...
  return [
    ...
    '3. A sufficiently understood new Agent is created immediately: call apply_workspace_changes with exactly one create_agent operation. Other consequential changes return a proposal; show it to the person, wait for their approval here, then call confirm_workspace_change with the proposalId.',
    ...
    `5. Admin-only, beyond the handoffs in 4: Agent avatars (Agent page, then Configure) and these Settings sections under ${adminUrl}: ${settings}. Send the link; results touching them carry links.admin.`,
    '6. Show each result\'s presentation.markdown, never presentation.slack. After creating or changing an Agent, give the person the receipt\'s links.admin and links.slack and tell them to mention it in Slack (@handle).',
    '',
    `Admin: ${admin}`,
  ].join('\n');
}
