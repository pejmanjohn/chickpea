...
const TOOL_DESCRIPTIONS: Record<WorkspaceManagementToolName, string> = {
  ...
  apply_workspace_changes: 'Apply typed Chickpea workspace configuration changes with durable idempotency and per-item outcomes. It does not execute actions in Slack Lists or connected services. One sufficiently understood base Agent must be created immediately as a standalone create_agent operation; do not propose it or ask for confirmation. Keep capability and follow-on configuration operations separate. Other confirmation-required configuration operations return one bound proposal; never add a separate conversational approval gate around it.',
