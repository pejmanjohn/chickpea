...
/** Shared with the bounded synthetic contract evaluation; the live tool uses the same values. */
export const scheduleActionDescription = 'Create, edit, pause, resume, disable, or run scheduled work in the current Slack Channel or one-to-one DM. Use for any clear future or recurring request, including phrasing such as “check again in 5 minutes,” without asking for approval. Channel delivery defaults to a new channel message, even when requested in a thread. Set delivery to thread only when the requester explicitly asks for future results in this thread, not merely for the creation acknowledgement here. Existing schedules retain their saved destination. Do not use this tool to delete scheduled work; deleting a routine uses the existing proposal and explicit-confirmation flow. The host derives the destination and addressed Agent from trusted Slack context.';

export function slackManagementInstruction(agentId: string): string {
  return [
    `This Slack conversation is routed to trusted acting Agent ID ${agentId}.`,
    ...
    'For Agent-design brainstorming or capability questions about Agent configuration involving services, connections, repositories, models, sandboxes, or schedules, call inspect_workspace before naming or recommending specific capabilities. Its connectors field is the setup catalog, not current access. Only currentAgent.effectiveConnections names ready connections eligible for this Agent; request selection and surface policy can narrow the current turn, and an empty array means none. Ground the answer in that result instead of answering from general knowledge or offering to inspect later. For an explicit request to connect a named service to this Agent, call prepare_connector_setup directly; that tool validates catalog availability and requester authority, so do not call inspect_workspace first. Give the requester its returned actionLinks, describe it only as a secure Chickpea link, and never ask for credentials in Slack.',
    'For scheduled work, inspect_workspace provides stable connection IDs, labels and purposes for the owning Agent. Declare only required ready enabled accounts via requiredConnectionAccountIds, including [] when no external connection is needed. Never copy every mounted account or substitute another account. On a task change, deliberately supply the updated account set; metadata-only edits preserve it. If the intended account is unclear, ask. inspect_routines includes the saved set.',
    ...
    'Standalone requests for future or repeated work belong to manage_scheduled_work, even when the requester does not use the word “schedule” (for example, “check this again in 5 minutes”). “Again” means create a fresh follow-up unless the requester explicitly identifies an existing routine to edit. Keep “in N minutes” relative by using scheduleKind in plus minutes; do not compute a wall-clock time. “Tell me anything new” implies outputPolicy post_on_change. Clear create, edit, pause, resume, disable, and run-now actions apply immediately without approval. Before acting on an existing routine, call inspect_routines and use an exact routine ID and current version where required; ask the requester to disambiguate if more than one routine matches. Deleting scheduled work is deliberately excluded from manage_scheduled_work because it is irreversible: for a clear request to delete a routine, first call inspect_routines, then send the exact delete_routine operation to propose_workspace_changes, show presentation.slack, and wait for explicit requester approval before calling confirm_workspace_change. Never use apply_workspace_changes for routine deletion. Apart from routine deletion, do not route standalone scheduled work through propose_workspace_changes or apply_workspace_changes. Compound Agent-configuration changes still use the normal proposal flow.',
  ].join(' ');
}
...
export function scheduleToolOperation(
  ...
  return {
    ...
    ...(data.requiredConnectionAccountIds !== undefined ? { requiredConnectionAccountIds: data.requiredConnectionAccountIds } : {}),
    schedule,
