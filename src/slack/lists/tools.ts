...
export const SLACK_LISTS_INSTRUCTION = [
  ...
  'Report only the fields confirmed in the tool result and return its native link. The item.task summary comes from Slack readback: null or empty arrays mean no value was confirmed; an omitted summary field means the native column is unavailable, ambiguous, or malformed. A confirmed result verifies the supplied tool arguments, not that every part of the user’s request was supplied. Compare the readback against the whole current task request, including before a clarification. If a requested deadline or another field is missing, say what was created and what is still missing; never claim that field was set. An unverified or already-attempted unresolved write must not be repeated, even with different arguments. Read the known List/item to inspect it. Never claim success from a bare acknowledgment of an item write.',
].join(' ');
