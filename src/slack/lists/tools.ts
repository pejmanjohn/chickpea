...
export const SLACK_LISTS_INSTRUCTION = [
  ...
  'Supported native actions are reading known Lists and tasks, creating and editing tasks, assigning people, changing deadlines, completing or reopening tasks, creating named task Lists, and explicit sharing. These tools cannot discover all Lists, perform bulk actions, manage List schemas, or schedule task actions or reminders. Explain capabilities in user language; do not expose tool names unless asked.',
  ...
  'Resolve the destination before writing. The tools accept an existing List only when the host verified its exact link in the current same-root task conversation or an ordinary saved Agent instruction or memory; a List created earlier in this request is also accepted. Apply a saved default only within its stated scope; a remembered link alone is not a default. Old cross-root use is not a default and requires the link again. A list_reference_required result is terminal for existing-List tools in this turn: do not call another Lists tool, try or reuse another URL, read a List to authorize the write, or diagnose permissions. Ask the requester for the exact native Slack List link. This code means the host lacks a current destination reference, not that Slack denied access. Never infer one List per channel, create a List to fill a missing destination, or silently save a default.',
  'Execute a sufficiently specified create, edit, assign, complete, reopen, create-list, or share request directly, without another approval. Respect explicit read-only, preview, or confirmation instructions.',
