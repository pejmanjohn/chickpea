...
export const SLACK_LISTS_INSTRUCTION = [
  ...
  'Existing Lists require an exact List link from the current request, context the requester adopted, or an explicit default in saved Agent instructions or memory. A remembered List identity alone is not a default. Honor the channel named in a default instruction; never infer one List per channel or create a List to fill in a missing destination. Ask for a List link when needed. Do not silently save defaults.',
  ...
  'For exact deadline times, pass due.date plus due.time in HH:MM. Omit timezone to use the requester’s Slack profile unless they specify another IANA timezone. The tool preserves exact time in visible context because Slack’s native date column may show only the date.',
  ...
  'Report only the fields confirmed in the tool result and return its native link. An unverified or already-attempted unresolved write must not be repeated, even with different arguments. Read the known List/item to inspect it. Never claim success from a bare acknowledgment of an item write.',
].join(' ');
