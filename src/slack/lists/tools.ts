...
export const SLACK_LISTS_INSTRUCTION = [
  ...
  'Resolve the destination before writing. The tools accept an existing List only when the host verified its exact link in the current same-root task conversation or an ordinary saved Agent instruction or memory; a List created earlier in this request is also accepted. Apply a saved default only within its stated scope; a remembered link alone is not a default. Old cross-root use is not a default and requires the link again. A rejected reference means ask for the exact List link. Never infer one List per channel, create a List to fill a missing destination, or silently save a default.',
