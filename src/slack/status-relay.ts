 ...
 * observed tool_start can never hit the local Map there. Relay the already
 ...
 */
export async function relayObservedToolStatus(instanceId: string, statusText: string): Promise<void> {
  ...
  try {
    ...
    await tagStateStub(env).observedToolStatus(instanceId, statusText);
  } catch {
