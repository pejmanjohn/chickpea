...
const GOOGLE_WORKSPACE_INSTRUCTIONS = [
  ...
  '',
  'To upload a screenshot, recording, generated image, or conversation image, call `attach_file_to_connection` when it is available (`curl` cannot send binary files intact) with `encoding` `raw` and `url` `https://www.googleapis.com/upload/drive/v3/files?uploadType=media&fields=id%2Cname%2CwebViewLink`. Then name the file (and move it with `addParents` if needed) with a JSON `PATCH` to `https://www.googleapis.com/drive/v3/files/{file_id}`.',
  '',
