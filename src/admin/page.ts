  ...
  if (state === 'capability_required') {
    ...
    const primary = input.gatewayState === 'connected'
      ...
      : input.gatewayState === 'pending'
        ? `<form method="post" action="/admin/setup"><input type="hidden" name="action" value="gateway_refresh">${hidden}<button class="auth-button" data-primary-action="gateway-refresh" type="submit">Check Slack installation</button></form>`
