...
export function renderSlackAuthorizationHandoffPage(
  ...
  return renderSlackJourneyPage({
    ...
    body: `<div class="auth-actions"><a class="auth-link" data-slack-authorization-link href="${safeTarget}" rel="noreferrer">Continue to Slack</a></div><script src="${script}" defer></script>`,
  });
}
...
export function renderSlackSignInPage(destination: string): string {
  ...
  return renderSlackJourneyPage({
    ...
    body: `<form method="post" action="/auth/slack/oidc/start"><input type="hidden" name="purpose" value="login"><input type="hidden" name="destination" value="${escapeHtml(safeDestination)}"><button class="auth-button slack-provider-button" type="submit"><span class="slack-provider-logo slack-logo-image" aria-hidden="true"></span>Continue with Slack</button></form>`,
  });
}
...
export function renderSlackAccessDeniedPage(input: {
  ...
  return renderSlackJourneyPage({
    ...
    body: `<form method="post" action="/auth/slack/oidc/start"><input type="hidden" name="purpose" value="${input.purpose}">${capability}<input type="hidden" name="destination" value="${escapeHtml(safeDestination)}"><button class="auth-button" type="submit">Try another Slack account</button></form>${setupScript}`,
  });
}
...
export function renderSlackOwnerCompletePage(destination: string): string {
  ...
  return renderSlackJourneyPage({
    ...
    body: `<div class="auth-actions"><a class="auth-link" href="${escapeHtml(safeDestination)}">Open Chickpea</a></div>`,
  });
}
...
export function renderSlackInvitationMismatchPage(workspace?: SlackAuthWorkspace): string {
  return renderSlackJourneyPage({
    ...
    body: '<div class="auth-actions"><a class="auth-link" href="/auth/slack/invite">Try another Slack account</a></div><script src="/auth/slack/invite/client.js" defer></script>',
  });
}
...
export function renderSlackSetupPage(input: {
  ...
  if (state === 'capability_required') {
    body = `<form id="slack-setup-open-form" method="post" action="/admin/setup"><input type="hidden" name="action" value="open">${hidden}${input.notice ? `<input type="hidden" name="notice" value="${escapeHtml(input.notice)}">` : ''}<button class="auth-button" data-primary-action="resume-private" type="submit">Continue private setup</button></form>`;
    ...
    const primary = input.gatewayState === 'connected'
      ? `<form method="post" action="/auth/slack/oidc/start">${hidden}<input type="hidden" name="purpose" value="first_owner"><button class="auth-button slack-provider-button" data-primary-action="gateway-owner" type="submit"><span class="slack-provider-logo slack-logo-image" aria-hidden="true"></span>Continue with Slack</button></form>`
      : input.gatewayState === 'pending' || input.gatewayState === 'error'
        ? `${input.gatewayState === 'error' ? '<p class="auth-help" role="status">We couldn’t check Slack installation. Your authorization is saved. Try checking again.</p>' : ''}<form method="post" action="/admin/setup"><input type="hidden" name="action" value="gateway_refresh">${hidden}<button class="auth-button" data-primary-action="gateway-refresh" type="submit">Check Slack installation</button></form><form method="post" action="/admin/setup"><input type="hidden" name="action" value="gateway_resume">${hidden}<button class="auth-button secondary" data-secondary-action="gateway-resume" type="submit">Open Slack authorization again</button></form>`
        : `<form method="post" action="/admin/setup"><input type="hidden" name="action" value="gateway_begin">${hidden}<button class="auth-button slack-provider-button" data-primary-action="gateway-install" type="submit"><span class="slack-provider-logo slack-logo-image" aria-hidden="true"></span>Add to Slack</button></form>`;
    ...
    body = `<form method="post" action="/admin/setup"><input type="hidden" name="action" value="inspect">${hidden}<button class="auth-button" data-primary-action="inspect-creation" type="submit">Inspect saved attempt</button></form>`;
    ...
    body = `<form method="post" action="/auth/slack/install/start">${hidden}<button class="auth-button" data-primary-action="install-app" type="submit">Continue to Slack</button></form>`;
    ...
    body = `<form method="post" action="/auth/slack/install/resume">${hidden}<button class="auth-button" data-primary-action="resume-approval" type="submit">Resume Slack installation</button></form>`;
    ...
    body = `<form method="post" action="/auth/slack/oidc/start"><input type="hidden" name="purpose" value="first_owner">${hidden}<button class="auth-button slack-provider-button" data-primary-action="claim-owner" type="submit"><span class="slack-provider-logo slack-logo-image" aria-hidden="true"></span>Sign in with Slack</button></form>`;
    ...
    body = `<div class="auth-actions"><a class="auth-link" href="${escapeHtml(destination)}">Open Chickpea</a></div>`;
  }
...
export function renderSlackRecoveryPage(options: {
  ...
  if (stage === 'complete') {
    return renderSlackJourneyPage({
      ...
      body: '<div class="auth-actions"><a class="auth-link" href="/auth/slack/sign-in?destination=%2Fadmin">Continue to Sign in with Slack</a></div>',
    });
  }
  if (stage === 'waiting_events') {
    return renderSlackJourneyPage({
      ...
      alert: options.error,
      body: '<form method="post" action="/admin/recovery"><input type="hidden" name="action" value="finalize"><button class="auth-button" type="submit">Verify and promote repair</button></form>',
    });
  }
  if (stage === 'credentials') {
    return renderSlackJourneyPage({
      ...
      body: `<form method="post" action="/admin/recovery">
        ...
        <button class="auth-button" type="submit">Encrypt credentials and authorize Slack</button>
      </form>`,
    });
  }
  return renderSlackJourneyPage({
    ...
    body: `<form method="post" action="/admin/recovery">
      ...
      <button class="auth-button" type="submit">Start 15-minute repair</button>
    </form>`,
  });
}
