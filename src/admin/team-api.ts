...
async function readJson(c: Context): Promise<unknown> {
  ...
  if (Number.isFinite(declared) && declared > MAX_TEAM_BODY_BYTES) return undefined;
  try {
    const raw = await c.req.text();
    if (new TextEncoder().encode(raw).byteLength > MAX_TEAM_BODY_BYTES) return undefined;
