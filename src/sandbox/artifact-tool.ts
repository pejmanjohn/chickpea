...
export function buildArtifactToolsInstruction(
  ...
  const image = [
    ...
    [
      ...
      'Images are retained privately for up to 24 hours within a bounded cache. When attachment fails and a savedImage handle is returned, use recover_image to retry delivery without another generation. To resend an image already visible in Slack, use its current img:N inventory handle, including when earlier tool results are unavailable. It can compress the original bytes; allowResize only when the user accepts smaller dimensions. Never pay to regenerate merely to retry an upload. An expired_or_unavailable result means the retained image is no longer available. If recover_image cannot read a conversation image, explain the returned access, format, or size limitation as a resend failure. output_requirements_not_met means the actual dimensions or transparency did not meet the request; do not claim completion.',
    ].join(' '),
  ].join('\n');
  return [
    ARTIFACT_TOOLS_CORE,
