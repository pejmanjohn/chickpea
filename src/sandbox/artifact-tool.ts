...
/** Mirrors `MAX_IMAGES_PER_RESPONSE` / `MAX_IMAGE_TOOL_OUTPUTS` in image-tool.ts, which imports from here. */
const MAX_IMAGES_PER_RESPONSE = 4;
const MAX_IMAGE_TOOL_OUTPUTS = 4;
...
export function buildArtifactToolsInstruction(
  ...
  const image = [
    ...
    [
      ...
      'The tool inspects generated outputs before staging, and may correct one defect within the image budget. Read each file’s inspection: checked is a visual assessment, not proof of pixel identity; unavailable or uncertain must never be described as visually verified. Disclose any remaining discrepancy. Dimensions and transparency come from the actual file. Report returned facts, never guessed settings. Outputs also appear in the next turn’s img:N listing with origin=agent.',
      'Images are retained privately for up to 24 hours within a bounded cache. When attachment fails and a savedImage handle is returned, use recover_image to retry delivery without another generation. Saved-image upload recovery can compress the original bytes; allowResize only when the user accepts smaller dimensions. Slack may transform newly uploaded images, so do not promise byte identity for an upload. To resend an image already visible in Slack, prefer its current img:N inventory handle: this reuses the original Slack file without reuploading, including when earlier tool results are unavailable. A requested filename is then only the link label; the original download filename remains unchanged. Explain that distinction when relevant. Never pay to regenerate merely to retry an upload. An expired_or_unavailable or original_file_unavailable result means that source is no longer available for recovery. If recover_image cannot read a conversation image, explain the returned access, format, or size limitation as a resend failure. output_requirements_not_met means the actual dimensions or transparency did not meet the request; do not claim completion.',
    ].join(' '),
  ].join('\n');
  return [
    ARTIFACT_TOOLS_CORE,
