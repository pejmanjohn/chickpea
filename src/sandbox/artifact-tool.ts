...
export function buildArtifactToolsInstruction(
  ...
  const image = [
    ...
    [
      `Call \`${GENERATE_IMAGE_TOOL}\` at most once per response; a second call returns reason limit and produces no image. When someone asks for several variations, options, or takes of one idea, make that one call with count set to how many they asked for (at most 4); each variation is attached as its own file and the result lists them under files.`,
