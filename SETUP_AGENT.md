...
9. Connect the coding agent you are working in, and any scripts, to the management MCP server, so the workspace can be managed from there as well as from Admin and Slack. Every deployment serves an OAuth-protected management MCP server at `https://<deployment>/mcp`, and the client signs in through Slack by itself. The fastest path is to paste `Connect my coding agent to my Chickpea using https://<deployment>/connect.md` into the agent; the public page `https://<deployment>/connect` shows that prompt, and Admin **Settings → MCP** (`/admin/settings/agents-clients`) has the exact snippet for Claude Code, Codex, Cursor, VS Code, Windsurf, Gemini CLI, claude.ai, ChatGPT, or generic JSON, with one-click install for Cursor and VS Code. From a shell, `npx chickpea-cli doctor https://<deployment>` confirms the public surface is up (it reports a 404 as unfinished setup until step 6 completes), `npx chickpea-cli mcp config https://<deployment>` prints the same snippets, and `npx chickpea-cli login https://<deployment>` gives scripts the same access. See [the workspace management runbook](docs/runbooks/workspace-management-mcp.md).
