...
A fresh deployment starts `unconfigured` and exposes only the private seven-day setup capability generated by the deploy command. The default setup journey is **Add to Slack** for Chickpea's shared unlisted app. The deployment creates a short-lived signed claim, the private gateway binds Slack's OAuth result to that exact deployment, and the deployment opens an authenticated outbound session. The operator never supplies a Slack configuration token, client secret, signing secret, Events URL, app-level token, or bot token in this lane.

**Use your own Slack app** is the complete independent alternative. Its compact path asks the installer for a short-lived Slack App Configuration access token from [Your App Configuration Tokens](https://api.slack.com/apps#:~:text=Your%20App%20Configuration%20Tokens); Chickpea sends that token to Slack once and never stores it. Slack also displays an `xoxe-` refresh token, which Chickpea does not need. If the installer cannot create the configuration token, **Can't create an app configuration token?** opens `/admin/setup/manual` in the same browser tab. That separate journey restores the illustrated manifest, Slack Event Subscriptions, and app-credential walkthrough. It accepts App ID, Client ID, Client Secret, Signing Secret, and the exported JSON manifest—but never a pasted bot token. Submitted credentials are encrypted before persistence, and validation errors clear every write-only field.

Both installation lanes bind one exact Slack workspace and validate the issued bot capabilities. The customer-owned lane additionally requires a signed Events API challenge before promoting its encrypted credential revision. The shared lane keeps the Slack token inside the private gateway; the deployment retains only its tenant-bound gateway identity and workspace binding.
...
MCP and Slack tools never accept provider keys, OAuth codes, connector headers, repository installation credentials, Slack bot tokens or signing secrets, or other secrets. A management operation instead creates a one-use setup link for one frozen action and target. The link expires after 24 hours; its raw capability exists only in the URL fragment, is exchanged for an HttpOnly browser session bound to that setup operation, and is scrubbed before navigation. The cookie uses the installation root path so a registered OAuth callback can resume the same setup. The link holder cannot change the requester, organization, target, scopes, or action. Catalog connection setup also requires the initiating user's Chickpea session before granting account and tool access.
