...
| v0.1.27 | 1 | Violet QA lane (sandbox profile, Durable Object migration `v10`) on the merged candidate (main + #180, #186, #189; Worker versions 2ccc9272 and 387f262b) plus earlier candidate builds for the other journeys: a 43.8-minute delegated `workspace_task` finished with bot-authored commits and a draft PR and delivered one fresh final answer while channel mentions and DMs were answered in about 10 s; a same-source redeploy mid-task reset the coding worker's Sandbox Durable Object and the worker reconnected and finished; with the coding sandbox switched off, a coding follow-up was still answered; two named workspaces, create/welcome/first-approve, instruction approval, a channel schedule, connector file attach with `connection_request`, and persistence across redeploys passed on earlier builds of the same release. Shared gateway protocol 1 unchanged. |
