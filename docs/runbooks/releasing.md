...
| v0.1.28 | 1 | Violet QA lane (sandbox profile, Durable Object migration `v11`, no new migration) on the release candidate before its merge: `06757df7` (Worker version 06787e1f, runtime identical to main `8ab70372`) for most journeys, then the final tree `df41c71a` (Worker version 1abeee5a, adding #224, #225 and #227). Real channel, DM and mention-free thread replies; `@Chickpea` Agent creation with one welcome and a repository proposal applied on the first `approve`; instruction proposal approved and read back exactly in Admin; a plain turn on a repository-granted Agent made no Sandbox calls; a delegated `workspace_task` opened a draft PR whose commit was authored and committed as the GitHub App bot, while the working indicator showed rotating progress and cleared when the answer posted; two named workspaces stayed isolated; a one-time routine opened a workspace and delivered on time; the workspace restored from its R2 checkpoint after 31 minutes idle; five concurrent streamed answers finished on their first attempt with no degradation. On the final tree: state, sign-in and delivery retained across a redeploy, a clean welcome (#227), a team API-connection read, and a file attached from a workspace. Shared gateway protocol 1 unchanged. |
| v0.1.29 | 1 | Cobalt QA lane (sandbox profile, Durable Object migration `v11`, no new migration) on the release commit `af9a25e4` (Worker version 5e61ccfb), after a deep regression pass of v0.1.28 on the same lane (Worker version 0449c5a2: core Slack journeys, 12-thread streaming bursts, connectors, routines, coding workspaces and their limits, Admin). On the release commit: the first mention 10 s after deploy readiness was answered in 14 s; sign-in, Agents and settings were retained across the redeploy; a DM, a channel mention and a delegated `workspace_task` draft PR authored by the GitHub App bot; Usage showed one row per channel and routine names instead of ids (#231); `workspace_list` kept two open named workspaces and the open limit after an Agent edit moved the thread to a new coordinator instance (#232); a Slack request for all-repositories access produced a valid grant and coding workspaces kept working (#233). Shared gateway protocol 1 unchanged. |
