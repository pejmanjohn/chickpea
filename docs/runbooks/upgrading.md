...
Download or clone the official `v0.1.6` source release into a dedicated tooling
directory, then run:

```sh
git clone --branch v0.1.6 --single-branch https://github.com/pejmanjohn/chickpea.git chickpea-upgrades
...
v0.1.6 supports guided upgrades from v0.1.5. For older installations, use
current tooling to apply each intermediate release in order:
v0.1.0 → v0.1.1 → v0.1.2 → v0.1.3 → v0.1.4 → v0.1.5 → v0.1.6. Only the reviewed incoming version is accepted.
...
```sh
npm run upgrade -- --to v0.1.6 --preflight
npm run upgrade -- --to v0.1.6
```
