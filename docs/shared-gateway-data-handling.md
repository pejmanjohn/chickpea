...
HTTP setup proves endpoint and key possession before marking the connection ready. Socket installations retain their live session/version checks. Registration is recoverable after a lost response; it does not change Slack OAuth grants. See [HTTP delivery](runbooks/gateway-http-delivery.md) for migration and verification boundaries. On a late Slack retry, Chickpea may post one content-neutral offline notice, suppressed once per channel for 5 minutes and once per event for 25 hours.
