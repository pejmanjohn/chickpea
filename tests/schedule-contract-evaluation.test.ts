...
test('real active schema, converter and pre-admission guards distinguish schema acceptance from authorized scheduling', async () => {
  ...
  assert.ok(report.results.some((r: any) => r.schema === 'rejected'));
...
test('transport failure has zero evaluable samples; missing task constraints fail model grading despite structural admission', async () => {
  ...
  assert.equal(invalid.calls[0].result.admission, 'accepted');
  const negative = corpus.cases.find((entry: any) => entry.id === 'negation');
  assert.equal((await sampleScheduleModel(negative, async () => ({ stopReason: 'stop', content: [] }))).passed, true);
  assert.equal((await sampleScheduleModel(negative, async () => ({ stopReason: 'toolUse', content: [{ type: 'toolCall', name: 'manage_scheduled_work', arguments: entry.arguments }] }))).passed, false);
