...
test('fresh TAG_STATE installs one Slack-native identity schema in place', () => {
  ...
  try {
    ...
    assert.match(sql, /identity_slack_oauth_attempts/);
    assert.match(sql, /identity_slack_events_proofs/);
    assert.match(sql, /browser_hash/);
    assert.match(sql, /candidate_revision/);
    assert.match(sql, /ambiguous_external_effect/);
    assert.match(sql, /ciphertext/);
    assert.match(sql, /rotation_epoch/);
    assert.doesNotMatch(sql, /bot_token|signing_secret|client_secret|state_value|browser_value/);
