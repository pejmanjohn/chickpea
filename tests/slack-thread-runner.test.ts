...
test('a runner yields its turns only for a different serving version', async () => {
  const ts = (await import('typescript')).default;
  const vm = await import('node:vm');
  const text = readFileSync(new URL('../src/slack/thread-runner.ts', import.meta.url), 'utf8');
  const file = ts.createSourceFile('thread-runner.ts', text, ts.ScriptTarget.Latest, true);
  const runnerClass = file.statements.find((node) =>
    ts.isClassDeclaration(node) && node.name?.text === 'SlackThreadRunner');
  assert.ok(runnerClass && ts.isClassDeclaration(runnerClass));
  const method = runnerClass.members.find((member) =>
    ts.isMethodDeclaration(member) && member.name.getText(file) === 'supersede');
  assert.ok(method);
  const Probe = vm.runInNewContext(ts.transpileModule(
    `class Probe { ${method.getText(file)} }\nProbe`,
    ...
    env: { version?: string };
    supersedes: Set<() => void>;
    supersede(versionId: unknown): Promise<{ superseded: number }>;
  };
  const probe = new Probe();
  let yielded = 0;
  probe.supersedes = new Set([() => { yielded += 1; }]);
  probe.env = { version: 'v1' };
  assert.equal((await probe.supersede('v1')).superseded, 0, 'the serving version keeps observing');
  assert.equal((await probe.supersede('')).superseded, 0);
  assert.equal((await probe.supersede(undefined)).superseded, 0);
  assert.equal(yielded, 0);
  probe.env = {};
  assert.equal((await probe.supersede('v2')).superseded, 0, 'no own version: nothing to compare');
  probe.env = { version: 'v1' };
  assert.equal((await probe.supersede('v2')).superseded, 1);
