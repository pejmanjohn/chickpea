...
test('a registry whose immutable history carries the removed ownership feature still reads', (context) => {
  // The second-host lane-ownership feature was removed. Its writes — an
  // `ownership_changed` audit event and `ownership`/`authorityOrigin` on a
  // target record — survive on disk in append-only revision snapshots. The
  // reader must tolerate them; if it does not, the operator's live registry
  // becomes unreadable. See REMOVED_OWNERSHIP_KEYS in environment-registry.mjs.
  const f = fixture();
  context.after(() => rmSync(f.parent, { recursive: true, force: true }));
  const current = readEnvironmentRegistry(registryOptions(f.root));
  const next = JSON.parse(stableEnvironmentJson(current));
  next.revision = current.revision + 1;
  next.targets.amber.ownership = 'remote';
  next.targets.amber.authorityOrigin = 'https://chickpea-amber-live.example.workers.dev';
  next.audit.push({
    event: 'ownership_changed', target: 'amber',
    at: new Date(NOW).toISOString(), registryRevision: next.revision,
  });
  const name = `revision-${String(next.revision).padStart(16, '0')}.json`;
  writeFileSync(join(f.root, 'revisions', name), JSON.stringify(next), { mode: 0o600 });
  // The reader accepts the historical snapshot and surfaces no ownership field.
  const status = readEnvironmentStatus(registryOptions(f.root));
  const amber = status.targets.find((t: { target: string }) => t.target === 'amber');
  assert.equal(amber.health, 'ready');
  assert.equal('ownership' in amber, false);
  assert.equal('authorityOrigin' in amber, false);
  // Normalizing such a record drops the removed keys rather than throwing.
  const normalized = normalizeTarget({
    ...targetRecord('amber', f.first.revision, f.parent),
    ownership: 'remote',
    authorityOrigin: 'https://chickpea-amber-live.example.workers.dev',
  });
  assert.equal('ownership' in normalized, false);
  assert.equal('authorityOrigin' in normalized, false);
});
