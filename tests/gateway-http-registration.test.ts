...
async function fixture(loss?:'prepare'|'activate'|'rollback'){
 ...
 let remote:any={protocolVersion:1,...binding,mode:'socket',revision:0};let lost=false;let prepares=0;let activations=0;let blockActivation=false;let legacy=false;
 ...
 return {settings,config,client,legacy(){legacy=true;},counts:()=>({prepares,activations}),block(value:boolean){blockActivation=value;},expire(){remote.candidate=undefined;},cleanup(){settings.close();config.close();}};
}
