...
import type { SlackArtifactInput } from '../src/slack/web-client-presenter.ts';
...
test('render_chart preserves the upload size hint in a structured failure', async () => {
  const tool = createChartArtifactTool({
    channel: 'C_BOUND',
    async postArtifact() { return { uploaded: false, reason: 'too-large', maxBytes: 700 * 1024 }; },
  });
  assert.deepEqual(await tool.run({ ...TOOL_RUN_CONTEXT, data: BOOKINGS }), {
    output: { uploaded: false, reason: 'too-large', maxBytes: 700 * 1024, filename: 'chart.png' },
  });
});

test('render_chart renders a PNG and posts it to the bound thread', async () => {
  const uploads: SlackArtifactInput[] = [];
  const tool = createChartArtifactTool({
    channel: 'C_BOUND',
  ...
  assert.equal(upload.channel, 'C_BOUND');
  assert.equal(upload.threadTs, '1782770400.000300');
...
test('render_chart reports a missing upload scope without throwing', async () => {
  ...
  assert.deepEqual(result, {
    output: { uploaded: false, reason: 'missing-scope', filename: 'bookings-by-exam.png' },
  });
});
