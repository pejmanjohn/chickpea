...
// The removed second-host lane-ownership feature wrote `ownership` and
// `authorityOrigin` onto some records and an `ownership_changed` audit event.
// The registry is an append-only immutable history, so those revisions live on
// disk forever. Tolerate them on read only: accept the keys and event, never
// write or act on them. Do not add new behavior here.
const REMOVED_OWNERSHIP_KEYS = Object.freeze(['ownership', 'authorityOrigin']);
...
export function normalizeTarget(input) {
  ...
  if (Object.keys(input).some((key) => ![...TARGET_KEYS, 'setupFlowUnprovenSince', 'installation', ...REMOVED_OWNERSHIP_KEYS].includes(key))) {
    throw fail('INVALID_TARGET_RECORD');
  }
  assertActiveTarget(input.target);
  if (input.role !== 'branch') throw fail('INVALID_TARGET_ROLE');
  const target = input.target;
  // Strip the removed feature's keys so a normalized record never carries them.
  const { ownership: _ownership, authorityOrigin: _authorityOrigin, ...portable } = input;
  const record = {
    ...portable,
...
function validateTarget(input, target, legacy = false) {
  ...
  if (!isRecord(input) || !exactKeys(input, keys, ['setupFlowUnprovenSince', 'installation', ...REMOVED_OWNERSHIP_KEYS])
