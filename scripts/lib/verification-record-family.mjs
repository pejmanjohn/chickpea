...
export function readRunFamily(file, source, maxDepth = 32) {
  ...
  for (let depth = 0; path; depth += 1) {
    ...
    records.push({ path, run, view: status(run, source) });
    child = run;
...
export function familyStatus(family) {
  ...
  return { current: current.view, complete: !family.missingAncestor && current.view.complete && ancestors.every((entry) => entry.view.complete),
    ancestors: ancestors.map((entry) => ({ path: entry.path, runId: entry.run.id, complete: entry.view.complete,
...
export function renderFamilyReport(view, renderCurrent) {
  ...
  for (const ancestor of view.ancestors) {
    lines.push(`- Historical parent ${ancestor.runId} at ${ancestor.path}: complete ${ancestor.complete}; unresolved cases ${ancestor.unresolvedCases.map((item) => `${item.id}/${item.result}`).join(', ') || 'none'}; pending cleanup ${ancestor.cleanupPending.join(', ') || 'none'}.`);
