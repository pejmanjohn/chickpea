// Public source export policy: which tracked paths and contents may leave this
// repository, and the fast checks that prove a commit is release-clean before
// anything is installed or built. Shared by `verify:hygiene` (seconds, before
// every merge) and `verify:oss-export` (the immutable archive gate).
...
export const liveVerifierDenyPatterns = [
...
export const forbiddenSourcePathRoots = [
  exportPath('.agents'),
  exportPath('.claude'),
  exportPath('.codex'),
  exportPath('.gstack'),
...
export const allowedAgentSkillPaths = new Set([
...
export const liveVerifierExportPolicy = Object.freeze({
  requiredPaths: new Set([
    ...
    exportPath('qa', 'live', 'runner-matrix', 'analysis.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'cli.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'fake.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'hooks.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'page.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'plan.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'record.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'spec.json'),
    exportPath('qa', 'live', 'runner-matrix', 'tail.ts'),
    exportPath('qa', 'live', 'runner-matrix', 'types.ts'),
...
export const forbiddenLiveVerifierArtifactPaths = [
...
// `docs/` is default-denied by `.gitignore`; every public document is
// un-ignored there deliberately. Private roots and private artifact shapes are
// refused here regardless, and private content is caught by the leak scan.
export const privateDocPathPatterns = [
  ['private plans', /^docs\/plans(?:\/|$)/i],
  ['deleted source tree', /^docs\/source(?:\/|$)/i],
  ['private evidence', /^docs\/(?:evidence|private|notes)(?:\/|$)/i],
  ['private artifact', /^docs\/.*(?:\.journal\.jsonl|\.log|\.snapshot\.json|\.target\.json|\.transcript\.txt)$/i],
];

export const forbiddenSourcePaths = new Set([
  exportPath('.worktreeinclude'),
  exportPath('design-qa.md'),
]);

export const forbiddenBinaryExtensions = new Set([
  '.avif',
  '.gif',
  '.heic',
  '.jpeg',
...
export const allowedLargeTextFiles = new Map();
...
function git(root, args, { input } = {}) {
  const result = spawnSync('git', ['-C', root, ...args], {
    input, stdio: ['pipe', 'pipe', 'pipe'], maxBuffer: MAX_BUFFER,
  });
  if (result.error || result.status !== 0) {
    const detail = result.error?.message ?? result.stderr?.toString('utf8') ?? '';
    throw new Error(`git ${args[0]} failed with exit ${result.status}: ${detail.trim()}`);
  }
  return result.stdout;
}

function decodeRecords(output, label) {
  const decoded = output.toString('utf8');
  if (!Buffer.from(decoded, 'utf8').equals(output)) throw new Error(`${label} contains a non-UTF-8 path`);
  const records = decoded.split('\0');
  if (records.pop() !== '') throw new Error(`${label} is not NUL-terminated`);
  return records;
}
...
function assertSafeEntries(entries, label) {
  ...
  for (const { mode, type, object, path } of entries) {
    if (!OBJECT_ID.test(object)) throw new Error(`Malformed git object id for ${path || '<empty path>'}: ${object}`);
    if (type !== 'blob' || (mode !== '100644' && mode !== '100755')) throw new Error(`${path}: unsupported tracked entry (${mode} ${type})`);
    if (
      path.length === 0 ||
      path.includes('\\') ||
    ...
    if (existing && existing !== path) throw new Error(`Tracked source paths alias on common filesystems: ${existing} and ${path}`);
    aliases.set(alias, path);
  }
}

/** Every blob a commit tracks, from `git ls-tree`; structural problems throw. */
export function readTrackedManifest(root, revision = 'HEAD') {
  const sourceCommit = git(root, ['rev-parse', '--verify', `${revision}^{commit}`]).toString('utf8').trim();
  if (!OBJECT_ID.test(sourceCommit)) throw new Error(`git rev-parse returned an invalid commit id: ${sourceCommit}`);
  const entries = decodeRecords(git(root, ['ls-tree', '-r', '-z', '--full-tree', sourceCommit]), 'Tracked source manifest').map((record) => {
    ...
    if (!mode || !type || !object || extra.length > 0) throw new Error(`Malformed git ls-tree metadata for ${path || '<empty path>'}`);
    return { mode, type, object, path };
  });
  assertSafeEntries(entries, 'Tracked source manifest');
  return { entries, sourceCommit };
}

/** Every path the index tracks, for scanning uncommitted work in place. */
export function readIndexManifest(root) {
  const entries = decodeRecords(git(root, ['ls-files', '-z', '--stage']), 'Index manifest').map((record) => {
    const separator = record.indexOf('\t');
    if (separator < 0) throw new Error(`Malformed git ls-files record: ${record}`);
    const [mode, object, stage, ...extra] = record.slice(0, separator).split(' ');
    const path = record.slice(separator + 1);
    if (!mode || !object || stage === undefined || extra.length > 0) throw new Error(`Malformed git ls-files metadata for ${path || '<empty path>'}`);
    if (stage !== '0') throw new Error(`${path}: unmerged index entry; resolve the conflict first`);
    const type = mode === '160000' ? 'commit' : mode === '120000' ? 'symlink' : 'blob';
    return { mode, type, object, path };
  });
  assertSafeEntries(entries, 'Index manifest');
  return { entries, sourceCommit: null };
}

/** Blob bytes for every entry in one `git cat-file --batch` round trip. */
export function readBlobs(root, entries) {
  const objects = [...new Set(entries.map(({ object }) => object))];
  const blobs = new Map();
  if (objects.length === 0) return blobs;
  const output = git(root, ['cat-file', '--batch'], { input: `${objects.join('\n')}\n` });
  let offset = 0;
  while (offset < output.length) {
    ...
    const [object, kind, sizeText] = header.split(' ');
    if (kind === 'missing' || kind === undefined) throw new Error(`git cat-file --batch cannot read ${object}`);
    const size = Number(sizeText);
    if (!Number.isSafeInteger(size) || size < 0) throw new Error(`git cat-file --batch returned an invalid size for ${object}`);
    const start = newline + 1;
    blobs.set(object, output.subarray(start, start + size));
    offset = start + size + 1; // Each object is followed by a newline.
  }
  return blobs;
}

/** Contents by path: the working tree when present, otherwise the index blob. */
export function readContents(root, entries, { workingTree = false } = {}) {
  ...
  for (const entry of entries) {
    ...
    if (workingTree) {
      ...
      let stat;
      try { stat = lstatSync(file); } catch { /* deleted in the working tree */ }
      if (stat?.isFile()) { contents.set(entry.path, readFileSync(file)); continue; }
    }
    fromBlobs.push(entry);
  }
  const blobs = readBlobs(root, fromBlobs);
  for (const entry of fromBlobs) contents.set(entry.path, blobs.get(entry.object));
  return contents;
}

export function isLiveVerifierPublicPath(path) {
  return liveVerifierExportPolicy.requiredPaths.has(path);
}

export function isForbiddenLiveVerifierArtifact(path) {
  return forbiddenLiveVerifierArtifactPaths.some((pattern) => pattern.test(path));
}

export function privateDocReason(path) {
  return privateDocPathPatterns.find(([, pattern]) => pattern.test(path))?.[0];
}

/** Paths that may never leave the repository, plus the live verifier's exact inventory. */
export function publicSourceManifestFindings(entries) {
  const findings = [];
  ...
  for (const path of liveVerifierExportPolicy.requiredPaths) if (!paths.has(path)) findings.push(`missing public verifier file: ${path}`);
  for (const { path } of entries) {
    if (isForbiddenLiveVerifierArtifact(path)) findings.push(`forbidden private verifier artifact: ${path}`);
    else if (path.startsWith('qa/live/') && !liveVerifierExportPolicy.requiredPaths.has(path)) findings.push(`unreviewed verifier file: ${path}`);
  }
  return findings;
}
...
export function docsIgnoreFindings(root, entries) {
  ...
  const result = spawnSync('git', ['-C', root, 'check-ignore', '--no-index', '-z', '--stdin'], {
    input: `${docs.join('\0')}\0`, stdio: ['pipe', 'pipe', 'pipe'], maxBuffer: MAX_BUFFER,
  });
  if (result.error || (result.status !== 0 && result.status !== 1)) {
    throw new Error(`git check-ignore failed with exit ${result.status}: ${result.stderr?.toString('utf8').trim() ?? result.error?.message}`);
  }
  return decodeRecords(result.stdout, 'check-ignore output').map((path) => `${path}: tracked although .gitignore denies it; un-ignore it deliberately or keep it private`);
}
...
/** Binary allowlist, size ceiling, NUL bytes, and denied private terms. */
export function leakScanFindings(entries, contents) {
  const findings = [];
  for (const { path: rel, type } of entries) {
    if (type !== 'blob') { findings.push(`${rel}: unsupported non-file entry (${type})`); continue; }
    const buffer = contents.get(rel);
    if (!buffer) { findings.push(`${rel}: no content available for scanning`); continue; }
    ...
    if (forbiddenBinaryExtensions.has(extension)) {
      ...
      if (!expectedHash) { findings.push(`${rel}: forbidden binary/image extension ${extension}`); continue; }
      const actualHash = sha256(buffer);
      if (actualHash !== expectedHash) findings.push(`${rel}: allowed binary hash mismatch (${actualHash})`);
      continue;
    }
    if (buffer.length > 5_000_000) {
      const expectedHash = allowedLargeTextFiles.get(rel);
      if (!expectedHash) { findings.push(`${rel}: file is too large for text leak scanning (${buffer.length} bytes)`); continue; }
      if (sha256(buffer) !== expectedHash) { findings.push(`${rel}: allowed large-text hash mismatch`); continue; }
    }
    if (buffer.includes(0)) { findings.push(`${rel}: binary content is not allowed in the OSS export`); continue; }
    const text = buffer.toString('utf8');
    for (const [label, pattern] of denyPatterns) if (pattern.test(text)) findings.push(`${rel}: matched denied term ${label}`);
    if (isLiveVerifierPublicPath(rel)) {
      for (const [label, pattern] of liveVerifierDenyPatterns) if (pattern.test(text)) findings.push(`${rel}: matched denied verifier value ${label}`);
    }
  }
  return findings;
}

/** Extract `git archive` of a commit; the caller removes the directory. */
export function extractArchive(root, sourceCommit) {
  const scratch = mkdtempSync(join(tmpdir(), 'chickpea-export-'));
  const result = spawnSync('sh', ['-c', 'git -C "$1" archive --format=tar "$2" | tar -x -C "$3"', 'source-export-policy', root, sourceCommit, scratch], { stdio: ['ignore', 'pipe', 'pipe'] });
  if (result.error || result.status !== 0) {
    rmSync(scratch, { recursive: true, force: true });
    throw new Error(`git archive extraction failed: ${result.stderr?.toString('utf8').trim() ?? result.error?.message}`);
  }
  return scratch;
}

/** The archive must contain exactly the tracked bytes, plus the substituted commit. */
export function archiveFindings(scratch, entries, contents, sourceCommit) {
  const findings = [];
  for (const { path: rel } of entries) {
    const file = join(scratch, rel);
    let stat;
    try { stat = lstatSync(file); } catch (error) { findings.push(`${rel}: tracked archive entry is missing (${error.code ?? error.message})`); continue; }
    if (!stat.isFile()) { findings.push(`${rel}: unsupported non-file archive entry`); continue; }
    const tracked = contents.get(rel);
    if (!tracked) continue;
    const expected = rel === 'release-source.json' ? Buffer.from(tracked.toString('utf8').replace('$Format:%H$', sourceCommit)) : tracked;
    if (!readFileSync(file).equals(expected)) findings.push(`${rel}: archived bytes differ from the tracked blob`);
  }
  return findings;
}

/** Node baseline, engine ranges, and package identity of the tree at `root`. */
export function packageMetadataFindings(root) {
  const findings = [];
  const packageJson = JSON.parse(readFileSync(join(root, 'package.json'), 'utf8'));
  const cliPackage = JSON.parse(readFileSync(join(root, 'packages/cli/package.json'), 'utf8'));
  const lock = JSON.parse(readFileSync(join(root, 'package-lock.json'), 'utf8'));
  if (readFileSync(join(root, '.nvmrc'), 'utf8').trim() !== NODE_BASELINE
    || [packageJson, cliPackage, lock.packages[''], lock.packages['packages/cli']].some((pkg) => pkg?.engines?.node !== NODE_ENGINE)) {
    findings.push('Node baseline and root/CLI engine ranges must match the verifying runtime policy');
  }
  if (packageJson.private !== true || !packageJson.description || packageJson.license !== 'Apache-2.0' || !packageJson.repository) {
    findings.push('package.json must remain private and include its source metadata');
  }
  for (const file of ['LICENSE', 'NOTICE']) if (!existsSync(join(root, file))) findings.push(`missing ${file}`);
  return findings;
}

/** Public setup material must never prompt for, embed, or transport a credential. */
export function authExportContractFindings(root) {
  const findings = [];
  const read = (...parts) => readFileSync(join(root, ...parts), 'utf8');
  const packageJson = JSON.parse(read('package.json'));
  const bindings = packageJson.cloudflare?.bindings ?? {};
  if (Object.keys(bindings).length !== 0) findings.push('Consumer Deploy metadata must not prompt for a Chickpea credential');
  const recovery = read('scripts', 'recover-auth.mjs');
  if (/\bfetch\s*\(/.test(recovery) || /node:https/.test(recovery)) findings.push('Slack recovery preflight must remain read-only with no HTTP transport');
  const identityTypes = read('src', 'identity', 'types.ts');
  if (!identityTypes.includes("Omit<Invitation, 'locatorHash'>") ||
    ...
    findings.push('Identity export summary must omit invitation, operation, setup, token, and session locators');
  }
  ...
  if (activeExampleKeys.length !== 0) findings.push('Cloudflare Deploy example must expose no active secret prompt');
  if (!/^COMPOSIO_API_KEY=$/m.test(environmentExample) || /^COMPOSIO_API_KEY=.+$/m.test(environmentExample)) {
    findings.push('OSS environment example must keep the Composio project key optional and empty');
  }
  for (const key of ['DO_NOT_TRACK', 'CHICKPEA_DISABLE_TELEMETRY', 'CHICKPEA_TELEMETRY_ENVIRONMENT']) {
    if (!new RegExp(`^${key}=$`, 'm').test(environmentExample) || new RegExp(`^${key}=.+$`, 'm').test(environmentExample)) {
      findings.push(`OSS environment example must expose ${key} as an optional empty value`);
    }
  }
  const wrangler = read('wrangler.jsonc');
  if (/"vars"\s*:/.test(wrangler)) findings.push('Cloudflare Deploy config must not expose runtime defaults as customer-editable fields');
  if (!/"binding"\s*:\s*"AUTH_DB"/.test(wrangler) || !/"migrations_dir"\s*:\s*"migrations\/better-auth"/.test(wrangler)) {
    findings.push('Cloudflare config must bind AUTH_DB to the reviewed Better Auth migrations');
  }
  const publicAuthCopy = [read('README.md'), read('docs', 'authentication.md')].join('\n');
  const managedConnectorCopy = read('docs', 'runbooks', 'composio-managed-connectors.md');
  if (!/Self-hosters opt in by supplying their own Composio project key/i.test(managedConnectorCopy)
    || !/Without that key the adapter is dormant/i.test(managedConnectorCopy)) {
    findings.push('OSS managed-connector docs must explain BYOK setup and no-key fallback');
  }
  if (/Cloudflare Access is the default|Choose \*\*new Zero Trust organization\*\*/i.test(publicAuthCopy)) {
    findings.push('Public authentication copy still describes Access as the fresh-install default');
  }
  if (/BETTER_AUTH_SECRET\s*=/.test(publicAuthCopy + example)) findings.push('Public setup material must not expose or request a Better Auth secret');
  return findings;
}
...
const REQUIRED_PACKAGED_FILES = [
  ...
  'scripts/verify-product-telemetry.mjs',
  'scripts/lib/product-telemetry-preflight.mjs',
  'scripts/lib/cf-smoke-telemetry.mjs',
  ...
  'docs/runbooks/gateway-http-delivery.md',
...
/** What `npm pack` would ship from `cwd`: required public files present, private shapes absent. */
export function packManifestFindings(cwd, entries) {
  const result = spawnSync('npm', ['pack', '--dry-run', '--json', '--ignore-scripts'], { cwd, encoding: 'utf8', maxBuffer: MAX_BUFFER });
  if (result.error || result.status !== 0) return [`npm pack manifest failed: ${(result.stderr || result.stdout || result.error?.message || '').trim()}`];
  let manifest;
  try { manifest = JSON.parse(result.stdout); } catch { return [`npm pack returned invalid JSON: ${result.stdout.trim()}`]; }
  const packageJson = JSON.parse(readFileSync(join(cwd, 'package.json'), 'utf8'));
  const files = new Set((manifest[0]?.files ?? []).map((entry) => entry.path));
  const declaredFiles = new Set(packageJson.files ?? []);
  const publicVerifierFiles = entries.map(({ path }) => path).filter(isLiveVerifierPublicPath);
  const missing = [...new Set([...REQUIRED_PACKAGED_FILES, ...publicVerifierFiles])].filter((path) => !files.has(path));
  const forbidden = [...files].filter((path) =>
    path === '.worktreeinclude' ||
    (path.startsWith('.claude/') && path !== '.claude/skills/chickpea-live-verification/SKILL.md') ||
    path.startsWith('.github/') ||
    path.startsWith('.githooks/') ||
    path.startsWith('design/') ||
    (path.startsWith('config/environments/qa/') && !['config/environments/qa/desired-state.json', 'config/environments/qa/workspace-recipe.json'].includes(path)) ||
    (path.startsWith('docs/') && privateDocReason(path.toLowerCase()) !== undefined) ||
    isForbiddenLiveVerifierArtifact(path) ||
    path.startsWith('tmp/'));
  return [
    ...REQUIRED_PACKAGE_ENTRIES.filter((path) => !declaredFiles.has(path)).map((path) => `missing package.json files entry: ${path}`),
    ...missing.map((path) => `missing required packaged file: ${path}`),
    ...forbidden.map((path) => `forbidden packaged file: ${path}`),
  ];
}

/**
 * Run every pre-install hygiene check against one commit (`revision`, via a
 * fresh `git archive`) or the index plus working tree (`workingTree`). Each
 * check reports findings and its duration; the first structural failure ends
 * the run. With `keepScratch`, the extracted archive is returned for callers
 * that continue with install/build/test and remove it themselves.
 */
export function inspectSource({ root, revision = 'HEAD', workingTree = false, keepScratch = false } = {}) {
  const checks = [];
  const state = { sourceCommit: null, entries: [], contents: new Map(), scratch: null, treeRoot: root };
  const check = (name, body) => {
    const started = performance.now();
    let findings;
    try { findings = body() ?? []; } catch (error) { findings = [error instanceof Error ? error.message : String(error)]; }
    checks.push({ name, findings, durationMs: Math.round(performance.now() - started) });
    return findings.length === 0;
  };
  const cleanup = () => { if (state.scratch && !keepScratch) rmSync(state.scratch, { recursive: true, force: true }); };
  try {
    const manifestOk = check(workingTree ? 'index manifest' : 'tracked manifest', () => {
      const manifest = workingTree ? readIndexManifest(root) : readTrackedManifest(root, revision);
      state.entries = manifest.entries;
      state.sourceCommit = manifest.sourceCommit;
    });
    if (!manifestOk) return finish(state, checks, keepScratch);
    check('public source manifest', () => publicSourceManifestFindings(state.entries));
    check('tracked docs un-ignored', () => docsIgnoreFindings(root, state.entries));
    const contentsOk = check('source contents', () => { state.contents = readContents(root, state.entries, { workingTree }); });
    if (!contentsOk) return finish(state, checks, keepScratch);
    if (!workingTree) {
      const archiveOk = check('archive integrity', () => {
        state.scratch = extractArchive(root, state.sourceCommit);
        state.treeRoot = state.scratch;
        return archiveFindings(state.scratch, state.entries, state.contents, state.sourceCommit);
      });
    ...
    check('package metadata', () => packageMetadataFindings(state.treeRoot));
    check('authentication export contract', () => authExportContractFindings(state.treeRoot));
    check('npm pack manifest', () => packManifestFindings(state.treeRoot, state.entries));
    return finish(state, checks, keepScratch);
  } catch (error) {
    cleanup();
    throw error;
  }
...
function finish(state, checks, keepScratch) {
  const ok = checks.every(({ findings }) => findings.length === 0);
  if (state.scratch && (!keepScratch || !ok)) {
    rmSync(state.scratch, { recursive: true, force: true });
    state.scratch = null;
  }
  return { ...state, checks, ok, durationMs: checks.reduce((total, { durationMs }) => total + durationMs, 0) };
}
