...
- `target.example.json` demonstrates a role-agnostic example overlay (not an active Fern registration) using aliases only. `env target <alias>` supplies one selected color in that shape, and `env attest <alias>` supplies its doctor snapshot. The environment registry supports `amber`, `cobalt`, and staged adoption of `violet`; existing two-lane registries remain valid. Each registered lane binds the same exact four-variant smoke inventory and a run selects one. That registry rejects `deep` and continuation roles. The public target schema and catalog remain role-agnostic so a later deep-qualification module can use them without redefining contracts.
